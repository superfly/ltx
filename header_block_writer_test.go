...
func TestHeaderBlockWriter(t *testing.T) {
	t.Run("PageDataOnly", func(t *testing.T) {
		...
		if err := w.WriteHeader(ltx.Header{
			...
			MaxTXID:      6,
			Timestamp:    2000,
			PreChecksum:  ltx.ChecksumFlag | 5,
			PostChecksum: ltx.ChecksumFlag | 6,
		}); err != nil {
			t.Fatal(err)
		}
...
func TestHeaderBlockWriter_Close(t *testing.T) {
	...
	t.Run("ErrHeaderSeek", func(t *testing.T) {
		...
		if err := w.WriteHeader(ltx.Header{Version: 1, PageSize: 1024, PageN: 1, Commit: 1, DBID: 1, MinTXID: 1, MaxTXID: 1, PostChecksum: ltx.ChecksumFlag | 1}); err != nil {
			t.Fatal(err)
...
func TestHeaderBlockWriter_WriteEventHeader(t *testing.T) {
	...
	t.Run("ErrSizeRequired", func(t *testing.T) {
		w := ltx.NewHeaderBlockWriter(createFile(t, filepath.Join(t.TempDir(), "ltx")))
		if err := w.WriteHeader(ltx.Header{Version: 1, PageSize: 1024, EventN: 1, PageN: 1, Commit: 1, EventDataSize: 1, DBID: 1, MinTXID: 1, MaxTXID: 1, PostChecksum: ltx.ChecksumFlag | 1}); err != nil {
			t.Fatal(err)
		} else if err := w.WritePageHeader(ltx.PageHeader{Pgno: 1}); err != nil {
			t.Fatal(err)
...
func TestHeaderBlockWriter_WritePageHeader(t *testing.T) {
	...
	t.Run("ErrPageNumberOutOfBounds", func(t *testing.T) {
		w := ltx.NewHeaderBlockWriter(createFile(t, filepath.Join(t.TempDir(), "ltx")))
		if err := w.WriteHeader(ltx.Header{Version: 1, PageSize: 1024, PageN: 1, Commit: 4, DBID: 1, MinTXID: 2, MaxTXID: 2, PreChecksum: ltx.ChecksumFlag | 2, PostChecksum: ltx.ChecksumFlag | 2}); err != nil {
			t.Fatal(err)
	...
	t.Run("ErrSnapshotInitialPage", func(t *testing.T) {
		w := ltx.NewHeaderBlockWriter(createFile(t, filepath.Join(t.TempDir(), "ltx")))
		if err := w.WriteHeader(ltx.Header{Version: 1, PageSize: 1024, PageN: 2, Commit: 2, DBID: 1, MinTXID: 1, MaxTXID: 2, PostChecksum: ltx.ChecksumFlag | 2}); err != nil {
			t.Fatal(err)
	...
	t.Run("ErrSnapshotNonsequentialPages", func(t *testing.T) {
		w := ltx.NewHeaderBlockWriter(createFile(t, filepath.Join(t.TempDir(), "ltx")))
		if err := w.WriteHeader(ltx.Header{Version: 1, PageSize: 1024, PageN: 3, Commit: 3, DBID: 1, MinTXID: 1, MaxTXID: 1, PostChecksum: ltx.ChecksumFlag | 1}); err != nil {
			t.Fatal(err)
		} else if err := w.WritePageHeader(ltx.PageHeader{Pgno: 1}); err != nil {
			t.Fatal(err)
	...
	t.Run("ErrOutOfOrderPages", func(t *testing.T) {
		w := ltx.NewHeaderBlockWriter(createFile(t, filepath.Join(t.TempDir(), "ltx")))
		if err := w.WriteHeader(ltx.Header{Version: 1, PageSize: 1024, PageN: 2, Commit: 2, DBID: 1, MinTXID: 2, MaxTXID: 2, PreChecksum: ltx.ChecksumFlag | 2, PostChecksum: ltx.ChecksumFlag | 2}); err != nil {
			t.Fatal(err)
		} else if err := w.WritePageHeader(ltx.PageHeader{Pgno: 2}); err != nil {
			t.Fatal(err)
...
func TestHeaderBlockWriter_WriteEventData(t *testing.T) {
	...
	t.Run("ErrSizeMismatch", func(t *testing.T) {
		w := ltx.NewHeaderBlockWriter(createFile(t, filepath.Join(t.TempDir(), "ltx")))
		if err := w.WriteHeader(ltx.Header{Version: 1, PageSize: 1024, EventN: 1, PageN: 1, Commit: 1, EventDataSize: 10, DBID: 1, MinTXID: 2, MaxTXID: 2, PreChecksum: ltx.ChecksumFlag | 2, PostChecksum: ltx.ChecksumFlag | 2}); err != nil {
			t.Fatal(err)
		} else if err := w.WritePageHeader(ltx.PageHeader{Pgno: 1}); err != nil {
			t.Fatal(err)
